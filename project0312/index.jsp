...
<%
  //jsp는 자바의 문법을 따르므로, 아래처럼 변수 선언 시 String 으로 선언
  //오라클 접속 정보

  //jsp 의 코드 영역은 서버에서 실행되어 버린다. 따라서 원본 그대로를
  //클라이언트한테 보내주지 않는다. 즉 아래의 코드를 수행한 후, 클라이언트인
  //웹 브라우저가 이해할 수 있는 형식은 html 형식으로 응답정보를 재구성하여 보낸다
  ...
  */
  ...
  //javaEE 는 javaSE 를 포함하기 떄문에 STANDARD 코드가 가능
  Class.forName("oracle.jdbc.driver.OracleDriver"); //드라이버로드
  out.print("드라이버 로드 성공<br>");

  Connection con = null; //접속 후 그정보를 가진 인터페이스 객체
  ...
  if(con ==null){
    ...
    rs = pstmt.executeQuery(); //select문 인 경우 그 결과를 가져온다.
    
    //rs가 최초에는 아무런 레코드도 가리키고 있지 않는다. 즉 커서가 꼭대기로 올라와있다.
    //따라서 필요에 따라 커서를원하는 위치 두고, 그 위치에 해당하는 레코드를 가져오자
