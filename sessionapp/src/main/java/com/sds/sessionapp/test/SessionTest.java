...
//서블릿으로 요청을 처리하기 위해 개발자가 doXXX 형으로 재정의 해야 한다.
//요청이 get방식이면 doGet() , post방식이면 doPost() 재정의 해야한다.
public class SessionTest extends HttpServlet{
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//클라이언트의 브라우저에 출력할 문자열을 response 객체의 출력스트림에 모으자
		PrintWriter out = response.getWriter();
		
		//아래의 문자열이 한글이 깨지지 않으려면, jsp에서의 page 지시영역에서 작성한 기능을
		//똑같이 작성해야 한다.
		response.setContentType("text/html;charset=utf-8");
		out.print("어서오세요 처음 뵐께요");
		
		//클라이언트가 현재 서블릿을 요청 시, 등짝(브라우저 메모리)에 아무것도 즉 쿠키ID없이 접근한다면
		//톰캣은 session 객체를 생성하고, 이 객체에 할당된 ID를 이용하여 쿠키값으로 응답정보
		//생성 시 살짝 껴 넣는다.
		...
		System.out.println("현재 요청한 클라이언트가 발급 받은 세션ID"+ sessionID); //서버의 로그에 출력
		
		//클라이언트가 아래의 사항에 해당하지만 않으면 쿠키값이 유지된다.
		//1) 웹 브라우저 종료
		//2) 정해진 시간 동안 요청 안하기
