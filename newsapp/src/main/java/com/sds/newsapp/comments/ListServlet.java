...
public class ListServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//HTTP 통신에서 객체 자체를 보낼 수 없다. 특히 아래의 코드는 그냥 변수이다 목록 그자체가 아니다.
		//HTTP 통신에서는, 모두 문자열로 취급하여, 게시물을 전송하려면 해당 게시물을 문자열화 시켜서
		//풀어서 보내줘야함
		//결론) List안에 들어있는 DTO들을 다꺼내어 그 안에 들어있는 데이터들을 문자열로 전송 해야 함.
		//out.print();
		
		//json 자체를 자바 코드에서 사용 불가하다. 왜? json은 문자열 데이터 포맷일 뿐 언어가 아니므로
		...
		for(int i=0;i<list.size();i++) {
			//댓글 수 만큼 반복문 돌리되, size-1 보다 작은 동안 만 쉼표로 구분해주고, 반대경운 쉼표x
			//리스트에서 DTO 꺼내기
