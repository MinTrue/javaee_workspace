	...
	function getCommentsList(){
		...
		xhttp.onreadystatechange=function(){
			if(this.readyState==4 && this.status==200){
				...
				//결론, json은 문자열이므로, 객체 자체가 아니다.  따라서 분석 할 수 없다 ex)배열의 길이 
				//배열 0번째 들어있는 객체가 가진 msg 변수 값
				//해결책) 넘어온 문자열을 분석하여 객체화 시키면 된다. 이 기능은 js 의 JSON 내장객체 가 지원
				
				obj = JSON.parse(this.responseText);
				
				//분석한 이후부터는, 서버로부터 넘겨 받은 문자열은 지금부터 객체로 취급
				console.log("게시물 수는",obj.commentsList.length);
				
				//obj.commentsList 테이블 태그를 동적으로 출력해보자
				...
				for(let i=0;i<obj.commentsList.length;i++){
						let json = obj.commentsList[i]; //배열로 부터 json 하나를 꺼낸다
						tag+="<tr>";
						tag+="<td>"+json.msg+"</td>";
