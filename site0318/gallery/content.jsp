...
<script>
  ...
  function del(){
    if(confirm("정말 삭제하시겠어요?")){
      ...
      //파일명과 galley_idx 두개이상을 전송해야 하므로, 아래쪽에 form 태그에
      //히든으로 파라미터들이  준비되어 있으므로 기존 폼을 전송하자
      //삭제 요청시엔 파일을 전송하는 갓이 아니므로, 
      // 복합데이터형식인 multipart/form-data 로 보낼 필요가 없다.
      let form1 = documente.getElementById("form1");

      //프로그래밍 적으로 encoding 방식으로  바꾸자
      form1.encoding=""; //js 에서는 enctype 속성이 encoding으로 처리해야 한다.
      form1.method="post"
