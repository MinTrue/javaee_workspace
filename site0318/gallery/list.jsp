...
<%
    ...
    String sql="select * from gallery order by gallery_idx desc"; // desc 내림차순
    pstmt = con.prepareStatement(sql); //쿼리수행 객체 준비

    //쿼리실행(select문이므로, 퀴리 실행 후 그 결과표 반환되어 ResultSet 으로 받자)
    rs =pstmt.executeQuery(); //select 문의 경우 executeQuery() 사용해야 한다.
...
<script>
  $(function(){
    ...
    $("button").click(function(){
      ...
      //defualt는 Get 방식이다. 우리가 흔이 사용해왓던 일반 링크느 사실 GET방식
      //요청 이였다
      //Oreilly 사  Apachi 사
      location="/gallery/write.jsp";
    });
  });
</script>
</head>
