...
<%
	...
	//참고로 java:comp/env 까지는 이미 정해진 이름이므로 누락해서는 안됨.
	context = new InitialContext(); //검색 객체
	
	//jndi 이름을 통해 구현체를 얻는다!
	ds = (DataSource)context.lookup("java:comp/env/jndi/oracle");
	
	Connection con = ds.getConnection(); //풀로부터 커넨션 객체 1개를 꺼내기
...
<%
	rs.close();
	pstmt.close();
	con.close(); //톰켓의 풀로부터 얻어온 Connection 을 반납하는 코드는 따로 있는게 아니라, 그냥 close()
