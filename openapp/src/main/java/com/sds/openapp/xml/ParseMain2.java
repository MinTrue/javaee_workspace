 ...
 * 없다. 즉 해석이나 분석의 과정이 필요한데, 이러한 해석 과정을 파싱(parsing)이라 한다.
 * 자바에서 xml파싱의 방법은 크게 2가지가 있다
 * 1) DOM 방식 - 태그마다 1:1 대응되는 객체를 메모리에 올려놓고 그 객체를 통해 문서를 간접 접근하는 방식
...
public class ParseMain2 {
	...
	public static void main(String[] args) {
		...
		//기준으로 가져와 보자 어떤 이득? 추후 이프로그램을 모든 os에서 실행할 수 있게 끔
		//즉 플랫폼 중립적으로 개발하기 위해
		...
		try {
			SAXParser parser = factory.newSAXParser();//파서 인스턴스
			parser.parse(uri.toString(), handler = new MyHandler());
			
			//결과 보고
			for(Member memver : handler.list) {
				//System.out.println("empno:"+member2.getEmpno()+", ename:"+member.getEname()+", sal="+member.getSal());
			}
