...
public class MedicHandler extends DefaultHandler {
	...
	boolean isXPos; //경도 XPos
	boolean isYPos; //위도 YPos
	boolean isyadmNm; //병원명 yadmNm
	
	Hospital dto;
	
	public MedicHandler(ListServlet lsitServlet) {
		this.listServlet=lsitServlet;
	}
	...
	//문서가 시작할때 호출되는 메서드
 	...
 	//여는 태그를 발견 했을때 호출되는 메서드
	public void startElement(String uri, String localName, String tag, Attributes attributes) throws SAXException {
		if(tag.equals("items")) {
			list = new ArrayList<Hospital>();
		} else if(tag.equals("item")) {
			dto = new Hospital(); //empty DTO 생성
		} else if(tag.equals("addr")) { //주소 시작 태그를 지나가면
			isAddr =true;
		}else if(tag.equals("XPos")) { //경도 시작 태그를 지나가면
			isXPos =true;
		}else if(tag.equals("YPos")) { //위도 시작 태그를 지나가면
			isYPos =true;
		}else if(tag.equals("yadmNm")) { //병원명 시작 태그를 지나가면
			isyadmNm =true;
		}	
	}
	...
	public void characters(char[] ch, int start, int length) throws SAXException {
		...
		//DTO에 data 담되, 현재 true로 열려있는 태그만
		if(isAddr) { 
	...
	public void endElement(String uri, String localName, String tag) throws SAXException {
		...
		} else if(tag.equals("addr")) { //주소 닫는 태그를 지나가면
			isAddr =false;
