...
public class ListServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("요청받음"+perPage);
		...
		String serviceKey="D0PNT7rXjpL1iOhBBcShigXfosW6tEpN3V9ID+iNICogV6ukytLuW97b3Gm7Tz1gkB0PsGkJxlFHOK+wrjEVbQ==";
		...
		HttpURLConnection con=(HttpURLConnection)urlCon; //다운캐스팅
		...
		if(result >=200 && result<=300) {
			...
			reader = new InputStreamReader(is); //문자기반으로 업그레이드(한 자씩 읽어들임 효율X)
			buffr = new BufferedReader(reader); //버퍼 스트림으로 업그레이드 (한 줄씩 읽어들임. 효울O)
